...
def download_token_images(target_folder):
    tokens_file = target_folder / "tokens.json"
    if not tokens_file.exists():
        print(f"tokens.json not found at {tokens_file}")
        print("Run download_tokens_json.py first.")
        return

    with open(tokens_file, "r") as f:
        all_tokens = json.load(f)

    images_dir = target_folder.parent / "tokens_fullres"
    ...
    print(f"Downloading {total} token images...\n")
    start_time = time.time()

    for token in all_tokens:
        name = sanitize_filename(token["name"])
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                sys.stdout.write(
                    f"\r\033[K  Failed ({response.status_code}): {token['name']}\n"
                )
                failed += 1
        except requests.RequestException as e:
            sys.stdout.write(f"\r\033[K  Error downloading {token['name']}: {e}\n")
            failed += 1
...
if __name__ == "__main__":
    ...
    print(f"Downloading images to {target_folder.parent / 'tokens_fullres'}")
    download_token_images(target_folder)
